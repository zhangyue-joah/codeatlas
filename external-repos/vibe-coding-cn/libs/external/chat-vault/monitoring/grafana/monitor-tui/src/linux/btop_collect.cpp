...
namespace AltData {
	...
	bool init() {
		...
		string path = string(home) + "/.projects/chat-vault/services/chat-vault/output/chat_history.db";
	...
	long long query_int(const string& sql) {
		if (!db && !init()) return 0;
	...
	std::unordered_map<string, long long> query_map(const string& sql) {
		std::unordered_map<string, long long> res;
		if (!db && !init()) return res;
		sqlite3_stmt* stmt = nullptr;
		if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
			while (sqlite3_step(stmt) == SQLITE_ROW) {
				const char* key = (const char*)sqlite3_column_text(stmt, 0);
				if (key) res[key] = sqlite3_column_int64(stmt, 1);
			}
		}
		if (stmt) sqlite3_finalize(stmt);
		return res;
...
namespace Cpu {
    ...
    static constexpr auto detect_active_cpus() {
        ...
        if (buf.empty()) {
            ...
            for (int i = 0; i < Shared::coreCount; ++i) result.push_back(i);
            return result;
        }
...
namespace Proc {
	...
	//* Collects from chat_history.db
	auto collect(bool no_update) -> vector<proc_info>& {
		if (Runner::stopping) return current_procs;
		if (no_update and not current_procs.empty()) return current_procs;
		...
		auto clean_text = [](const string& s) {
			string out;
			out.reserve(s.size());
			auto append_utf8 = [&](char32_t x) {
				if (x < 0x80) out.push_back(static_cast<char>(x));
				else if (x < 0x800) {
					out.push_back(static_cast<char>(0xC0 | (x >> 6)));
					out.push_back(static_cast<char>(0x80 | (x & 0x3F)));
				} else if (x < 0x10000) {
					out.push_back(static_cast<char>(0xE0 | (x >> 12)));
					out.push_back(static_cast<char>(0x80 | ((x >> 6) & 0x3F)));
					out.push_back(static_cast<char>(0x80 | (x & 0x3F)));
				} else {
					out.push_back(static_cast<char>(0xF0 | (x >> 18)));
					out.push_back(static_cast<char>(0x80 | ((x >> 12) & 0x3F)));
					out.push_back(static_cast<char>(0x80 | ((x >> 6) & 0x3F)));
					out.push_back(static_cast<char>(0x80 | (x & 0x3F)));
				}
			};
			...
			// 允许的ASCII符号: . , ! ? : ; ' " / ( ) [ ] + % $ = @ &
			auto is_allowed_ascii = [](char32_t c) {
			...
			for (size_t i = 0; i < s.size();) {
				...
				// ASCII: 只允许白名单字符
				if (cp < 0x80) {
					if (is_allowed_ascii(cp)) out.push_back(static_cast<char>(cp));
					continue;
				}
				// CJK 汉字
				if (cp >= 0x4E00 && cp <= 0x9FFF) { append_utf8(cp); continue; }
				// 中文标点白名单
				if (cp == 0x3002 ||  // 。
				    cp == 0xFF0C ||  // ，
				    cp == 0xFF01 ||  // ！
				    ...
				    cp == 0x2026)    // …
				{
					append_utf8(cp); continue;
				}
			}

			string final; bool last_space = true;
			for (char ch : out) {
				if (ch == ' ') {
					if (!last_space) { final += ch; last_space = true; }
				} else { final += ch; last_space = false; }
			}
			return final;
		};

		// 查询 chat_history.db 的 sessions 表，提取最后一条消息的 content 值
		string sql = "SELECT file_path, start_time, source, "
					 "substr(messages, instr(messages, '\"content\": \"') + 13, 800), "
					 "token_count, session_id "
					 "FROM sessions ";
		if (!filter.empty()) sql += "WHERE (source LIKE '%" + filter + "%' OR messages LIKE '%" + filter + "%') ";
		sql += "ORDER BY start_time DESC LIMIT 500";

		sqlite3_stmt* stmt = nullptr;
		int row_num = 0;
		if (sqlite3_prepare_v2(AltData::db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
			while (sqlite3_step(stmt) == SQLITE_ROW) {
				proc_info p;
				p.pid = ++row_num;

				// start_time -> short_cmd (显示时间)
				const char* ts = (const char*)sqlite3_column_text(stmt, 1);
				if (ts) {
					string ts_str = ts;
					size_t t_pos = ts_str.find('T');
					if (t_pos != string::npos && ts_str.length() > t_pos + 8) p.short_cmd = ts_str.substr(t_pos + 1, 8);
					else p.short_cmd = ts_str.substr(0, 8);
				}

				// source -> user/name (claude/codex/gemini/kiro)
				const char* source = (const char*)sqlite3_column_text(stmt, 2);
				p.user = source ? source : "";
				p.name = p.user;

				// messages content -> cmd (消息内容，保留头尾截断中间)
				const char* content = (const char*)sqlite3_column_text(stmt, 3);
				string raw = content ? content : "";
				// 截断：保留前150 + ... + 后150
				const size_t HEAD = 150, TAIL = 150;
				if (raw.length() > HEAD + TAIL + 10) {
					raw = raw.substr(0, HEAD) + " ... " + raw.substr(raw.length() - TAIL);
				}
				p.cmd = clean_text(raw);
				if (p.cmd.empty()) p.cmd = "<empty>";

				// token_count -> threads
				p.threads = sqlite3_column_int(stmt, 4);

				p.state = 'R';
				current_procs.push_back(std::move(p));
			}
			sqlite3_finalize(stmt);
		}

		numpids = (int)current_procs.size();
