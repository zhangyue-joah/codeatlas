---
slug: claude-code
title: "Claude Code"
seoTitle: "Claude Code：面向多步任务的终端 AI 编程 Agent（2026）"
seoDescription: "Claude Code 是一款终端优先的 AI 编程 Agent，擅长多步任务与仓库级改动，是在 AI 编程导航中承载“计划→补丁→验证”工作流的核心工具之一。"
description: "终端优先的 AI 编程 Agent，适合多步任务与仓库级改动。"
logo: /logos/placeholder.svg
website: "https://docs.claude.com/en/docs/claude-code"
productType:
  - cli
category:
  - agent-coding
targetUsers:
  - "前端"
  - "后端"
  - "全栈"
notFor:
  - "强合规/离线环境且无法接入的团队（需先评估）"
capabilities:
  - repo-context
  - read-only
useCases:
  - "日常编码：补全/解释/生成常见样板"
  - "快速验证：用小任务跑通一个闭环"
pros:
  - "上手快"
  - "能显著节省重复劳动"
cons:
  - "复杂任务仍需拆解与验收"
  - "团队落地需要权限/审计与数据策略"
onboardingTime: "10-30 分钟"
dependencies:
  - "网络可用（按工具策略）"
  - "Git 基础操作（如涉及仓库改动）"
pricing:
  model: usage-based
  free: false
  hasEducation: false
  hasTeam: false
  hasEnterprise: false
  supportsInvoice: false
  supportsContract: false
purchase:
  officialUrl: "https://docs.claude.com/en/docs/claude-code"
  sources:
    -
      label: "Claude Code 文档"
      url: "https://docs.claude.com/en/docs/claude-code"
  updatedAt: "2025-12-28"
privacy:
  dataRetention: "以官方政策为准"
  localProcessing: false
  enterpriseCompliance: []
  policyUrl: "https://www.anthropic.com/legal/privacy"
  updatedAt: "2025-12-28"
relatedTools: []
relatedTutorials:
  - claude-code-getting-started
relatedCompares: []
updatedAt: "2025-12-28"
version: "2025-12"
keywords:
  - "Claude Code"
  - Anthropic
  - "AI 编程"
  - Agent
  - "终端"
  - AI 编程导航
  - AI 编程导航站
---



## 一句话建议

- 先用“只读/建议模式”跑通一个小任务，再逐步开放写入与执行权限（如适用）。
- 价格/条款以官方为准；团队场景优先走可追溯采购路径。

## 购买与合规提示

- 优先官方/授权渠道；企业场景尽量使用可开票/可签合同的路径。
- 对第三方聚合/路由平台：评估审计、数据边界与 SLA。
